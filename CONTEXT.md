...
**Cancelled By**:
The actor that moved an Appointment to `cancelled`: `user` (see Pembatalan Mandiri), `staff` (e.g. Resepsionis, or Admin Klinik resolving a `perlu_dijadwal_ulang` Appointment), or `system` (the Slot Hold scheduler, or Tutup Layanan's end-of-day auto-cancel). Fixed at the moment of cancellation and never rewritten afterward, mirroring how Booked By is fixed at creation — recorded for reporting (e.g. how much cancellation volume is User self-service versus Staff-handled).
...
**Pembatalan Mandiri**:
A User cancelling their own `confirmed` Appointment themselves, through their booking history — distinct from a Staff-initiated cancellation (e.g. Tutup Layanan's auto-cancel, or Admin Klinik resolving a `perlu_dijadwal_ulang` Appointment) and from Pembatalan Visit Status, which reverts a Visit Status stage rather than the Appointment itself. Only available up to a fixed, platform-wide cutoff: the Appointment's own date, at 00:00 in the Klinik's local timezone (see Jam Operasional) — from that moment the option is withdrawn and the User is directed to contact the Klinik's Resepsionis instead, the same fallback-to-Resepsionis pattern as an ineligible Self Check-In. Confirmed to the User in-app only; unlike Kode Check-In, nothing needs to be retrieved again afterward, so no WhatsApp message is sent. Restricted to User accounts — a guest Patient has no authenticated session to prove ownership of the Appointment, and still cancels only through a Resepsionis. Covers cancellation only, not changing to a different Slot — that remains Reschedule, which stays Staff-only for now. See [ADR 0034](docs/adr/0034-appointment-self-cancellation-cutoff-is-fixed-platform-wide.md).
_Avoid_: Cancel (English term already covered by the Appointment Status value `cancelled`; Pembatalan Mandiri names the User-initiated *path* to it, not the status); do not conflate with Pembatalan Visit Status, which is a different concept entirely.

**Alasan Pembatalan**:
An optional free-text reason a User may give when performing a Pembatalan Mandiri, the same optional-capture pattern as Keluhan.
